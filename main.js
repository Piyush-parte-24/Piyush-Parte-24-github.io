...
(function () {
  ...
  if (!reduceMotion) {
    ...
    if (trex) {
      ...
      /* ============================================================
       EXTRA CINEMATIC FX — Fog + Resin Ripple + Pterosaur Flock
       ============================================================ */

    /* ---- 1. Layered Fog on Hero ---- */
    if (!reduceMotion) {
      const heroScene = $(".hero-scene");
      if (heroScene) {
        const fogContainer = document.createElement("div");
        fogContainer.className = "hero-fog";

        for (let i = 0; i < 3; i++) {
          const fog = document.createElement("div");
          fog.className = "fog-layer";
          fog.style.animationDelay = `-${i * 12}s`;
          fogContainer.appendChild(fog);
        }
        heroScene.appendChild(fogContainer);
      }
    }

    /* ---- 2. Amber Resin Ripple on Card Click ---- */
    if (!reduceMotion) {
      const addRipple = (el) => {
      ...
      $$(".spec-card, .record:not(.locked)").forEach(addRipple);

      // Also apply to dynamically created cards (archive)
      const observer = new MutationObserver(() => {
        $$(".spec-card, .record:not(.locked)").forEach(el => {
    ...
    /* ---- 3. Procedural Pterosaur Flock with Wing Physics ---- */
    if (!reduceMotion && !isMobile) {
      ...
      if (hero) {
        ...
        for (let i = 0; i < flockCount; i++) {
          ...
          ptero.className = "ptero-enhanced";
          ptero.style.left = Math.random() * 100 + "%";
          ptero.style.top = (15 + Math.random() * 45) + "%";
          ptero.style.width = (22 + Math.random() * 18) + "px";
          ptero.style.height = "auto";
          ptero.style.opacity = (0.45 + Math.random() * 0.35).toFixed(2);

          ptero.innerHTML = `
            <svg viewBox="0 0 48 18" width="100%" height="100%">
              <g>
                <!-- Body -->
                <ellipse class="body" cx="24" cy="9" rx="11" ry="4.5"/>
                <!-- Left Wing -->
                <path class="wing left-wing" d="M24 9 Q14 3 6 11" fill="none" stroke="rgba(6,8,6,.75)" stroke-width="2.5" stroke-linecap="round"/>
                <!-- Right Wing -->
                <path class="wing right-wing" d="M24 9 Q34 3 42 11" fill="none" stroke="rgba(6,8,6,.75)" stroke-width="2.5" stroke-linecap="round"/>
              </g>
            </svg>
          `;

          hero.appendChild(ptero);

          // Store data for animation
          flock.push({
            el: ptero,
            x: parseFloat(ptero.style.left),
            y: parseFloat(ptero.style.top),
            speed: 0.08 + Math.random() * 0.12,
            flapSpeed: 1.2 + Math.random() * 0.8,
        ...
        function animateFlock() {
          ...
          flock.forEach((p, index) => {
            p.x += p.speed;
            if (p.x > 108) p.x = -8;

            // Gentle bobbing + wind
            const bob = Math.sin(time * 0.8 + index) * 0.8;
            p.y = parseFloat(p.el.style.top) + bob * 0.15;

            p.el.style.left = `${p.x}%`;
            p.el.style.top = `${p.y}%`;

            // Wing flapping physics
            const flap = Math.sin(time * p.flapSpeed + p.phase) * 18;
            const leftWing = p.el.querySelector(".left-wing");
            const rightWing = p.el.querySelector(".right-wing");

            if (leftWing) leftWing.setAttribute("d", `M24 9 Q14 ${9 - flap} 6 11`);
            if (rightWing) rightWing.setAttribute("d", `M24 9 Q34 ${9 - flap} 42 11`);
          });
